          ...
          <div class="flex items-center gap-2">
            ...
            <div class="relative flex-1 sm:flex-initial">
              ...
              <div
                ...
                <div
                  @click="activeCategory = 'all'; categoryMenuOpen = false; currentPage = 1"
                ...
                <div
                  ...
                  :key="opt.value"
                  @click="activeCategory = opt.value; categoryMenuOpen = false; currentPage = 1"
                  class="px-4 py-2.5 text-xs text-gray-600 hover:bg-[#4A6FA5]/5 hover:text-[#4A6FA5] cursor-pointer transition-all duration-150">
                  {{ opt.label }}
                </div>
              </div>
          ...
          <table class="w-full min-w-[880px] border-collapse">
            <thead>
              <tr class="bg-gray-50/50 border-b border-gray-100">
                <th class="text-left px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-gray-400 tracking-wide uppercase">Product</th>
                <th class="text-left px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-gray-400 tracking-wide uppercase">Product ID</th>
                <th class="text-left px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-gray-400 tracking-wide uppercase">Description</th>
                <th class="text-left px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-gray-400 tracking-wide uppercase">Category</th>
                <th class="text-left px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-gray-400 tracking-wide uppercase">Status</th>
                <th class="text-right px-4 sm:px-5 py-3 sm:py-4 text-[10px] sm:text-xs font-semibold text-gray-400 tracking-wide uppercase">Actions</th>
              </tr>
            </thead>
            <tbody>
              <template v-for="product in displayedProducts" :key="product.id">
                ...
                <!-- ===================== EDIT MODE ROW ===================== -->
                <tr v-if="product.id === editingId" class="bg-[#4A6FA5]/5 border-b border-gray-50 last:border-b-0">
                  <td colspan="6" class="px-4 sm:px-5 py-4 sm:py-5">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <textarea
                        ...
                        class="w-full h-20 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-600 placeholder-gray-300 outline-none focus:border-[#4A6FA5] transition resize-none sm:col-span-2"></textarea>
                      ...
                      <div class="h-10 rounded-lg border border-gray-200 px-3 flex items-center focus-within:border-[#4A6FA5] transition sm:col-span-2">
                        <input
                          v-model="tagsInput"
                          type="text"
                ...
                <!-- ===================== VIEW MODE ROW ===================== -->
                <tr v-else class="border-b border-gray-50 last:border-b-0 hover:bg-gray-50/70 transition-all duration-150 align-top">
                  <!-- Product -->
                  <td class="px-4 sm:px-5 py-4 sm:py-5">
                    <p class="text-sm font-semibold text-[#1a2332]">{{ product.name }}</p>
                  ...
                  <td class="px-4 sm:px-5 py-4 sm:py-5">
                    <div class="flex items-center gap-1.5">
                      ...
                      <span class="text-[11px] text-gray-500 font-mono uppercase tracking-wide">{{ product.sku }}</span>
                    </div>
                  </td>

                  <!-- Description -->
                  <td class="px-4 sm:px-5 py-4 sm:py-5 max-w-xs">
                    <p class="text-xs text-gray-400 leading-relaxed line-clamp-2">{{ product.description }}</p>
                  </td>
  ...
  // ---- Seed data (only this data is used, nothing invented) ----
  const products = ref([
    {
      id: 1,
